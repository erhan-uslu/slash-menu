...
<?php $kategori_con=$db->prepare("SELECT * FROM kategoriler  where  id=?");  
...
$kategori_cek=$kategori_con -> fetch(PDO:: FETCH_ASSOC); 
 if ($kategorisorgula==0) {
   header('Location:'.$siteyolu);
    ...
    top: 0;
    left: 0;
    background: linear-gradient(180deg, rgb(11 2 2 / 0%) 70%, #000000db 100%);
}
					</style>
						   <a href="category?masa_id_no=<?php echo $_GET['masa_id_no']; ?>" class="btn btn-danger mb-2"><i class="fa fa-arrow-left"  style="top:0!important;"></i></a>
					<?php  $kategoricount = 0;
...
<div class="col-sm-12">
 ...
 <?php    
       ...
       while ($kategoricek=$datacon2 -> fetch(PDO:: FETCH_ASSOC)){   ?>
					 		<li> <a href="alt-kategoriler-<?php echo $kategoricek['seo_url'] ?>?masa_id_no=<?php echo $_GET['masa_id_no']; ?>&id=<?php echo $kategoricek['id'] ?>"><div class="item list-item-full bg-gradiant-black pl-0 pr-0">
 							 				<img src="timthumb?src=<?php echo $siteyolu; ?><?php echo  $kategoricek['ust_kategori_resim']; ?>&w=200&h=130&zc=4&a=tc" alt="image"> 
							 			 
							 		</div> 
	<a href="alt-kategoriler-<?php echo $kategoricek['seo_url'] ?>?masa_id_no=<?php echo $_GET['masa_id_no']; ?>&id=<?php echo $kategoricek['id'] ?>">	<h5 class=" fw-600 mb-0" style="  color : #111; text-align: center;
...
<div class="col-sm-12">
					 		 <div class="item-header fw-600 text-black mt-3 mb-3"  style="text-align: center;">
                              
                               <h4><?php echo $kategori_cek['kategori_ad']  ?>
</h4> </div>
					 		<ul class="shop-item pl-0">
 
                              <?php      $uruncon=$db->prepare("SELECT * FROM urunler where urun_kategori=?  and durum=1 and kat_id=".$kat_id." order by id asc  ");
